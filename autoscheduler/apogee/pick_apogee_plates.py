...
def pick_plates(apg, obs, par, times, lengths, schedule):
	...
	for c in range(len(chosen[:,0])):
		...
		picks.append({'obsmjd': thistime-2400000, 'exposure_length': thislength, 'plate': thisplate, 'first_backup': thisbak1, 'second_backup': thisbak2, 'visits_expected': apg[chosen[c,0]].vplan, 'visits_complete': apg[chosen[c,0]].vdone, 'sn': apg[chosen[c,0]].sn})
