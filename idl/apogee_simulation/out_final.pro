...
if par.sim eq 1 then begin
	...
	; No simulation, individual nights. Print out final results for diagnostics.
	openw, lun, "output/apogee.txt", /get_lun
	printf, lun, apg[i].name, apg[i].ra, apg[i].dec, apg[i].plateid, apg[i].apgver, apg[i].ha, apg[i].manpriority, apg[i].vplan, apg[i].vdone, sqrt(apg[i].sn), apg[i].minha, apg[i].maxha, apg[i].priority, apg[i].hist, format='(A-12, 2F14.5, I8, I8, F8.1, 3I8, 3F8.1, F12.1, 5X, A)'
	free_lun, lun
endelse
