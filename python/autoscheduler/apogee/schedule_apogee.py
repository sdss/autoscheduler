...
def schedule_apogee(schedule, errors, plan=False, loud=True, twilight=False):
	...
	if twilight:
		...
		if schedule['manga_end'] > schedule['eboss_end']: 
			times= [schedule['manga_end']+(par['exposure'] + par['overhead']) / 60 / 24]
		else:
			times= [schedule['eboss_end']+(par['exposure'] + par['overhead']) / 60 / 24]
		lengths = [(par['exposure'] + par['overhead']) / 60]
		#temp result until we want to really test twilight
		return []
