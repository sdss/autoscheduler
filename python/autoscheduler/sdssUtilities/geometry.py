...
class Angle(object):
    ...
    def _degrees(self, deg):
        """
        """
        if isinstance(deg, "".__class__): self.degrees = convert.sex2dec(*convert.parseDec(deg))
    ...
    def _hours(self, hr):
        """
        """
        if isinstance(hr, "".__class__): self.hours = convert.sex2dec(*convert.parseRA(hr))
        else: self.hours = hr
    ...
    def _setUnits(self, units):
        if units == 'degrees':
            if not self._customBounds: self._bounds = (-180., 180.)
            else: print "You specified custom bounds for this object. If you are changing units, you may need to respecify those bounds in the new units."
            self._str = self.degrees__str__
            self._repr = self.degrees__repr__
            self._value = self.degrees
        elif units == 'hours':
            if not self._customBounds: self._bounds = (0., 24.)
            else: print "You specified custom bounds for this object. If you are changing units, you may need to respecify those bounds in the new units."
            self._str = self.hours__str__
            self._repr = self.hours__repr__
            self._value = self.hours
        elif units == 'radians':
            if not self._customBounds: self._bounds = (0., 2*math.pi)
            else: print "You specified custom bounds for this object. If you are changing units, you may need to respecify those bounds in the new units."
    ...
    def normalize(self):
        ...
        if self._value < self.bounds[0]:
            ...
            if self.units == 'degrees': self._degrees(self._value)
            elif self.units == 'radians': self._radians(self._value)
            elif self.units == 'hours': self._hours(self._value)
...
class RA(Angle):
    ...
    @staticmethod
    def fromUnits(val, units):
        if units.lower() == 'degrees': return RA(degrees=val)
        elif units.lower() == 'hours': return RA(hours=val)
        elif units.lower() == 'radians': return RA(radians=val)
        else: raise ValueError("Invalid units!")
    ...
    def __init__(self, **kwargs):
        ...
        if kwargs.has_key("degrees"):
            self.bounds = (0, 360.)
        elif kwargs.has_key("radians"):
            self.bounds = (0, 2.*math.pi)
        elif kwargs.has_key("hours"):
            self.bounds = (0, 24.)
...
class Dec(Angle):
    ...
    @staticmethod
    def fromUnits(val, units):
        if units.lower() == 'degrees': return Dec(degrees=val)
        elif units.lower() == 'hours': return Dec(hours=val)
        elif units.lower() == 'radians': return Dec(radians=val)
        else: raise ValueError("Invalid units!")
...
class EquatorialCoordinates(CoordinateSystem):
    ...
    def __init__(self, ra, dec, **kwargs):
        ...
        if kwargs.has_key('z'):
            ...
            self.distance = None
        elif kwargs.has_key('redshift'):
            self.z = float(kwargs['redshift'])
            self.distance = None
        elif kwargs.has_key('distance'):
            if not kwargs.has_key('distanceUnits'):
                raise ValueError("If you specify 'distance', you must also specify 'distanceUnits' to be one of:\npc, kpc, mpc, ly, m, or km.")
